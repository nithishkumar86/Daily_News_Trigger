...
const FIXED_JOB_TOPICS = ['all', 'hiring', 'layoffs', 'jobs', 'hr']
...
function getTabClass(active: boolean): string {
  return `min-h-[44px] py-2 px-5 rounded-full text-base font-medium transition-all whitespace-nowrap ${
    active ? 'bg-[#f59e0b] text-white' : 'bg-[#1a1a2e] text-[#94a3b8] hover:bg-[#1e293b]'
  }`
}
...
export default function JobPage() {
  ...
  const topics = useMemo(
    () => [...FIXED_JOB_TOPICS, ...dbTopics.filter(t => !FIXED_JOB_TOPICS.includes(t))],
    [dbTopics]
  )
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('job-news-page')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'job_hire_fire' }, loadItems)
      .subscribe()
    ...
    <div className="min-h-screen bg-[#0d0d1a] pb-32 sm:pb-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
        ...
        <div className="mb-6 sm:mb-8 pt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#f1f5f9]">Job News</h1>
            <p className="text-[#94a3b8] mt-1 text-sm sm:text-base">Company hiring, layoffs, and workforce moves</p>
          </div>
          {hasItems && (
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 sm:py-28 text-[#94a3b8]">
            ...
            {activeTab !== 'all' ? (
              ...
              <>
                <p className="text-base sm:text-lg font-medium text-center">No job news yet today.</p>
